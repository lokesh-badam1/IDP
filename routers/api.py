...
def post_extract_details(db:Annotated[Session,Depends(get_db)],file: UploadFile = File(...),digitization:bool = Form(False)):

    if file.content_type not in ["application/pdf","image/jpeg","image/png"]:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,detail="Unsupported file type. Only PDF, JPEG, and PNG are allowed.")
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,detail="Failed to process invoice")
